...
// @desc    Save an individual answer
...
export const saveAnswer = async (req, res) => {
  try {
    ...
    // Update the Map of userAnswers
    session.userAnswers.set(questionId, selectedOption);
    await session.save();

    res.status(200).json({ success: true, userAnswers: session.userAnswers });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server Error" });
  }
